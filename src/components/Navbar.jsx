  ...
  return (
    <nav className="bg-gradient-to-r from-gray-900 via-indigo-950 to-gray-900 shadow-2xl p-4 relative sticky top-0 z-50 border-b border-indigo-500/30">
      <div className="container mx-auto flex items-center relative">
        {/* Futuristic Logo with Neon Pulse */}
        ...
        {/* Hamburger with Holographic Spin */}
        <button
          onClick={toggleMenu}
        ...
        >
          <div className="relative w-6 h-6">
            <FaBars
              size={24}
              className={`absolute top-0 left-0 text-cyan-400 transition-all duration-500 transform ${
                isMenuOpen
        ...
        {/* Desktop Navigation with Holographic Hover */}
      ...
      {/* Mobile Menu with No Gap */}
      <div
        className={`fixed top-[64px] right-0 w-full h-[calc(100vh-64px)] bg-gradient-to-br from-gray-900 via-indigo-950 to-purple-950 transform ${
          isMenuOpen ? "translate-x-0" : "translate-x-full"
        } transition-transform duration-700 ease-in-out z-40 p-6 lg:hidden overflow-y-auto`}
      >
        <ul className="flex flex-col items-center mt-12 space-y-10 text-3xl font-futura text-cyan-300">
          {navItems.map(({ label, href, title }, index) => (
            <li
              key={label}
              className="w-full flex justify-center relative animate-warp-in"
        ...
        {/* Social Icons without Backgrounds or Animations */}
